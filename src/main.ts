...
import { handleOliveAgentProReturn } from "./main/pro";
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("oliveagent", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("oliveagent");
}
...
export async function onReady() {
  ...
  if (settings.enableAutoUpdate) {
    ...
    const host = `https://api.oliveagent.sh/v1/update/${postfix}`;
    logger.info("Auto-update release channel=", postfix);
    updateElectronApp({
      logger,
      updateSource: {
        type: UpdateSourceType.ElectronPublicUpdateService,
        repo: "oliveagent-sh/oliveagent",
        host,
      },
...
function handleDeepLinkReturn(url: string) {
  // example url: "oliveagent://supabase-oauth-return?token=a&refreshToken=b"
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (parsed.protocol !== "oliveagent:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected oliveagent://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  // oliveagent://oliveagent-pro-return?key=123&budget_reset_at=2025-05-26T16:31:13.492000Z&max_budget=100
  if (parsed.hostname === "oliveagent-pro-return") {
    const apiKey = parsed.searchParams.get("key");
    if (!apiKey) {
      dialog.showErrorBox("Invalid URL", "Expected key");
      return;
    }
    handleOliveAgentProReturn({
      apiKey,
    });
    // Send message to renderer to trigger re-render
    mainWindow?.webContents.send("deep-link-received", {
  ...
  // oliveagent://add-mcp-server?name=Chrome%20DevTools&config=eyJjb21tYW5kIjpudWxsLCJ0eXBlIjoic3RkaW8ifQ%3D%3D
  ...
  // oliveagent://add-prompt?data=<base64-encoded-json>
