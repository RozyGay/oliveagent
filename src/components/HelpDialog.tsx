...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const isOliveAgentProUser = settings?.providerSettings?.["auto"]?.apiKey?.value;
...
## System Information
- OliveAgent Version: ${debugInfo.oliveagentVersion}
...
`;
      ...
      const labels = ["bug"];
      if (isOliveAgentProUser) {
        labels.push("pro");
      }
      const githubIssueUrl = `https://github.com/oliveagent-sh/oliveagent/issues/new?title=${encodedTitle}&labels=${labels}&body=${encodedBody}`;
      ...
      IpcClient.getInstance().openExternalUrl(
        "https://github.com/oliveagent-sh/oliveagent/issues/new",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://upload-logs.oliveagent.sh/generate-upload-url",
        {
          method: "POST",
          headers: {
...
`;
          ...
          <div className="space-y-4 overflow-y-auto flex-grow">
            ...
            <div className="border rounded-md p-3">
              ...
              <div className="text-sm bg-slate-50 dark:bg-slate-900 rounded p-2 max-h-32 overflow-y-auto">
                <p>OliveAgent Version: {chatLogsData.debugInfo.oliveagentVersion}</p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Need help with OliveAgent?</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col space-y-4 w-full">
          {isOliveAgentProUser ? (
            <div className="flex flex-col space-y-2">
              ...
              >
                <SparklesIcon className="mr-2 h-5 w-5" /> Chat with OliveAgent help
                bot (Pro)
              </Button>
              <p className="text-sm text-muted-foreground px-2">
                Opens an in-app help chat assistant that searches through OliveAgent's
                docs.
              </p>
            </div>
          ) : (
