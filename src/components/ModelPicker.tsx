import { isOliveAgentProEnabled, type LargeLanguageModel } from "@/lib/schemas";
...
export function ModelPicker() {
  ...
  const autoModels =
    !loading && modelsByProviders && modelsByProviders["auto"]
      ? modelsByProviders["auto"].filter((model) => {
          if (
            settings &&
            !isOliveAgentProEnabled(settings) &&
  ...
  if (settings && isOliveAgentProEnabled(settings)) {
          ...
          <>
            ...
            {primaryProviders.map(([providerId, models]) => {
              models = models.filter((model) => {
                // Don't show free models if OliveAgent Pro is enabled because
                // we will use the paid models (in OliveAgent Pro backend) which
