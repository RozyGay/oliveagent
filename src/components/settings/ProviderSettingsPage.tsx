...
export function ProviderSettingsPage({ provider }: ProviderSettingsPageProps) {
  ...
  const isOliveAgent = provider === "auto";
  ...
  // Use fetched data (or defaults for OliveAgent)
  const providerDisplayName = isOliveAgent
    ? "OliveAgent"
    : (providerData?.name ?? "Unknown Provider");
  const providerWebsiteUrl = isOliveAgent
    ? "https://academy.oliveagent.sh/settings"
    : providerData?.websiteUrl;
  const hasFreeTier = isOliveAgent ? false : providerData?.hasFreeTier;
  const envVarName = isOliveAgent ? undefined : providerData?.envVarName;
  ...
  const handleSaveKey = async (value: string) => {
    ...
    try {
      ...
      if (isOliveAgent) {
        settingsUpdate.enableOliveAgentPro = true;
      }
  ...
  // --- Toggle OliveAgent Pro Handler ---
  const handleToggleOliveAgentPro = async (enabled: boolean) => {
    setIsSaving(true);
    try {
      await updateSettings({
        enableOliveAgentPro: enabled,
      });
    } catch (error: any) {
      showError(`Error toggling OliveAgent Pro: ${error}`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen px-8 py-4">
      <div className="max-w-4xl mx-auto">
        ...
        {isOliveAgent && !settingsLoading && (
          <div className="mt-6 flex items-center justify-between p-4 bg-(--background-lightest) rounded-lg border">
            <div>
              <h3 className="font-medium">Enable OliveAgent Pro</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Toggle to enable OliveAgent Pro
              </p>
            </div>
            <Switch
              checked={settings?.enableOliveAgentPro}
              onCheckedChange={handleToggleOliveAgentPro}
              disabled={isSaving}
            />
          </div>
        )}
