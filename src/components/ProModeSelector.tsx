...
import { hasOliveAgentProKey, type UserSettings } from "@/lib/schemas";
...
export function ProModeSelector() {
  ...
  const toggleProEnabled = () => {
    updateSettings({
      enableOliveAgentPro: !settings?.enableOliveAgentPro,
    });
  };

  const hasProKey = settings ? hasOliveAgentProKey(settings) : false;
  const proModeTogglable = hasProKey && Boolean(settings?.enableOliveAgentPro);
  ...
  return (
    <Popover>
      <Tooltip>
        ...
        <TooltipContent>Configure OliveAgent Pro settings</TooltipContent>
      </Tooltip>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <h4 className="font-medium flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-primary font-medium">OliveAgent Pro</span>
            </h4>
          ...
          {!hasProKey && (
            <div className="text-sm text-center text-muted-foreground">
              <Tooltip>
                ...
                <TooltipContent>
                  Visit oliveagent.sh/pro to unlock Pro features
                </TooltipContent>
              </Tooltip>
            </div>
          )}
          <div className="flex flex-col gap-5">
            <SelectorRow
              id="pro-enabled"
              label="Enable OliveAgent Pro"
              tooltip="Uses OliveAgent Pro AI credits for the main AI model and Pro modes."
              isTogglable={hasProKey}
              settingEnabled={Boolean(settings?.enableOliveAgentPro)}
              toggle={toggleProEnabled}
            />
            <SelectorRow
              id="web-search"
              label="Web Access"
              tooltip="Allows OliveAgent to access the web (e.g. search for information)"
