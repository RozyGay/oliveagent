...
interface ErrorBannerProps {
  error: { message: string; source: "preview-app" | "oliveagent-app" } | undefined;
  onDismiss: () => void;
  onAIFix: () => void;
}
...
const ErrorBanner = ({ error, onDismiss, onAIFix }: ErrorBannerProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Add a little chip that says "Internal error" if source is "oliveagent-app" */}
      {error.source === "oliveagent-app" && (
      ...
      <div
        className={cn(
          "px-6 py-1 text-sm",
          error.source === "oliveagent-app" && "pt-6",
        )}
      >
        <div
        ...
        <div className="relative p-2 bg-red-100 dark:bg-red-900 rounded-sm flex gap-1 items-center">
          ...
          <span className="text-sm text-red-700 dark:text-red-200">
            ...
            {isDockerError
              ...
              : error.source === "oliveagent-app"
                ? "Try restarting the OliveAgent app or restarting your computer to see if that fixes the error."
...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
    ...
    if (!selectedComponentsPreview || selectedComponentsPreview.length === 0) {
      if (iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          { type: "deactivate-oliveagent-component-selector" },
          "*",
        );
      }
      setIsPicking(false);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "oliveagent-component-selector-initialized") {
        setIsComponentSelectorInitialized(true);
        return;
      }

      if (event.data?.type === "oliveagent-component-selected") {
      ...
      if (event.data?.type === "oliveagent-component-deselected") {
  ...
  const handleActivateComponentSelector = () => {
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-oliveagent-component-selector"
            : "deactivate-oliveagent-component-selector",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (
    !data ||
    data.type !== "oliveagent-component-selected" ||
