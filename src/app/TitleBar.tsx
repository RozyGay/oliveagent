...
import { OliveAgentProSuccessDialog } from "@/components/OliveAgentProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showOliveAgentProSuccessDialog = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "oliveagent-pro-return") {
        await refreshSettings();
        showOliveAgentProSuccessDialog();
        clearLastDeepLink();
      }
    };
    handleDeepLink();
...
export function OliveAgentProButton({
  ...
  return (
    <Button
      data-testid="title-bar-oliveagent-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isOliveAgentProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isOliveAgentProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isOliveAgentProEnabled && (
