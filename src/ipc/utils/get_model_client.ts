...
import { createOliveAgentEngine } from "./llm_engine_provider";
...
const oliveagentEngineUrl = process.env.OLIVEAGENT_ENGINE_URL;
...
export async function getModelClient(
  ...
  const oliveagentApiKey = settings.providerSettings?.auto?.apiKey?.value;
  ...
  // Handle OliveAgent Pro override
  if (oliveagentApiKey && settings.enableOliveAgentPro) {
    // Check if the selected provider supports OliveAgent Pro (has a gateway prefix) OR
    // we're using local engine.
    // IMPORTANT: some providers like OpenAI have an empty string gateway prefix,
    // so we do a nullish and not a truthy check here.
    if (providerConfig.gatewayPrefix != null || oliveagentEngineUrl) {
      const enableSmartFilesContext = settings.enableProSmartFilesContextMode;
      const provider = createOliveAgentEngine({
        apiKey: oliveagentApiKey,
        baseURL: oliveagentEngineUrl ?? "https://engine.oliveagent.sh/v1",
      ...
      logger.info(
        `\x1b[1;97;44m Using OliveAgent Pro API key for model: ${model.name} \x1b[0m`,
      );

      logger.info(
        `\x1b[1;30;42m Using OliveAgent Pro engine: ${oliveagentEngineUrl ?? "<prod>"} \x1b[0m`,
      );
      ...
      logger.warn(
        `OliveAgent Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
...
function getRegularModelClient(
  ...
  switch (providerId) {
    ...
    case "cody": {
      const provider = createOpenAICompatible({
        name: "cody",
        baseURL: "http://34.28.145.62:4000/v1",
        apiKey,
      });
      return {
        modelClient: {
