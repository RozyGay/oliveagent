...
function getEnvFilePath({ appPath }: { appPath: string }): string {
  return path.join(getOliveAgentAppPath(appPath), ENV_FILE_NAME);
}
...
export async function updateDbPushEnvVar({
  ...
  try {
    ...
    // Update or add OLIVEAGENT_DISABLE_DB_PUSH
    const existingVar = envVars.find(
      (envVar) => envVar.key === "OLIVEAGENT_DISABLE_DB_PUSH",
    );
    if (existingVar) {
      existingVar.value = disabled ? "true" : "false";
    } else {
      envVars.push({
        key: "OLIVEAGENT_DISABLE_DB_PUSH",
