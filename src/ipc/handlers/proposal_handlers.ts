...
import {
  ...
  getOliveAgentChatSummaryTag,
  getOliveAgentCommandTags,
...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getOliveAgentAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getOliveAgentChatSummaryTag(messageContent);

        const proposalWriteFiles = getOliveAgentWriteTags(messageContent);
        const proposalSearchReplaceFiles =
          getOliveAgentSearchReplaceTags(messageContent);
        const proposalRenameFiles = getOliveAgentRenameTags(messageContent);
        const proposalDeleteFiles = getOliveAgentDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getOliveAgentExecuteSqlTags(messageContent);
        const packagesAdded = getOliveAgentAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getOliveAgentWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getOliveAgentCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getOliveAgentChatSummaryTag(messageToApprove.content);
