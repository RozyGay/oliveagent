  ...
  handle(
    ...
    async (event, req: TokenCountParams): Promise<TokenCountResult> => {
      ...
      let systemPrompt = constructSystemPrompt({
        aiRules: await readAiRules(getOliveAgentAppPath(chat.app.path)),
      ...
      if (chat.app) {
        const appPath = getOliveAgentAppPath(chat.app.path);
        const { formattedOutput, files } = await extractCodebase({
          appPath,
          chatContext: validateChatContext(chat.app.chatContext),
        });
        codebaseInfo = formattedOutput;
        if (settings.enableOliveAgentPro && settings.enableProSmartFilesContextMode) {
          codebaseTokens = estimateTokens(
            files
              // It doesn't need to be the exact format but it's just to get a token estimate
              .map(
                (file) => `<oliveagent-file=${file.path}>${file.content}</oliveagent-file>`,
              )
              .join("\n\n"),
          );
        } else {
