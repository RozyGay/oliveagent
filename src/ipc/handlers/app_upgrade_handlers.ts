...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the OliveAgent component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://oliveagent.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl: "https://oliveagent.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes("@oliveagent/react-vite-component-tagger");
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("oliveagentComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [oliveagentComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @oliveagent/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @oliveagent/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[oliveagent] add OliveAgent component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[oliveagent] add Capacitor for mobile app support",
    });
