    ...
    const result = await db
      ...
      .where(
        and(
          ...
          like(messages.content, "%<oliveagent-security-finding%"),
        ),
      )
...
function parseSecurityFindings(content: string): SecurityFinding[] {
  ...
  // Regex to match oliveagent-security-finding tags
  // Using lazy quantifier with proper boundaries to prevent catastrophic backtracking
  const regex =
    /<oliveagent-security-finding\s+title="([^"]+)"\s+level="(critical|high|medium|low)">([\s\S]*?)<\/oliveagent-security-finding>/g;
