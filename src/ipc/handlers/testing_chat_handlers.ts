...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <oliveagent-add-dependency packages="deno"></oliveagent-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <oliveagent-add-dependency packages="@angular/does-not-exist"></oliveagent-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <oliveagent-add-dependency packages="react-router-dom react-query"></oliveagent-add-dependency>
  
  EOM`,
  write: `Hello world
  <oliveagent-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[oliveagent-qa=([^\]]+)\]/);
  if (match) {
