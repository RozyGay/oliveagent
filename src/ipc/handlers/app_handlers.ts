...
import { getOliveAgentAppPath, getUserDataPath } from "../../paths/paths";
...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official OliveAgent templates) should
      ...
      if (urlMatch) {
        proxyWorker = await startProxy(urlMatch[1], {
          onStarted: (proxyUrl) => {
            safeSend(event.sender, "app:output", {
              type: "stdout",
              message: `[oliveagent-proxy-server]started=[${proxyUrl}] original=[${urlMatch[1]}]`,
              appId,
            });
          },
        });
...
async function executeAppInDocker({
  ...
  const containerName = `oliveagent-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.oliveagent");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.oliveagent", "-t", `oliveagent-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `oliveagent-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
      "PNPM_STORE_PATH=/app/.pnpm-store",
      "-w",
      "/app",
      `oliveagent-app-${appId}`,
      "sh",
      "-c",
...
export function registerAppHandlers() {
  handle("restart-oliveagent", async () => {
    app.relaunch();
    app.quit();
  });
  ...
  handle(
    ...
    async (
      ...
      const fullAppPath = getOliveAgentAppPath(appPath);
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getOliveAgentAppPath(originalApp.path);
      const newAppPath = getOliveAgentAppPath(newAppName);
  ...
  handle("get-app", async (_, appId: number): Promise<App> => {
    ...
    // Get app files
    const appPath = getOliveAgentAppPath(app.path);
    let files: string[] = [];
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getOliveAgentAppPath("$APP_BASE_PATH"),
    };
  });
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes oliveagent-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (oliveagent-pnpm-${appId}).`,
                );
              } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = getOliveAgentAppPath(app.path);
        const newAppPath = getOliveAgentAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    const oliveagentAppPath = getOliveAgentAppPath(".");
    if (fs.existsSync(oliveagentAppPath)) {
      await fsPromises.rm(oliveagentAppPath, { recursive: true, force: true });
      // Recreate the base directory
      await fsPromises.mkdir(oliveagentAppPath, { recursive: true });
    }
