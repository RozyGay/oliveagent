...
import {
  ...
  getOliveAgentExecuteSqlTags,
  getOliveAgentSearchReplaceTags,
...
export async function dryRunSearchReplace({
  ...
  const oliveagentSearchReplaceTags = getOliveAgentSearchReplaceTags(fullResponse);
  for (const tag of oliveagentSearchReplaceTags) {
...
export async function processFullResponseActions(
  ...
  const appPath = getOliveAgentAppPath(chatWithApp.app.path);
  ...
  try {
    // Extract all tags
    const oliveagentWriteTags = getOliveAgentWriteTags(fullResponse);
    const oliveagentRenameTags = getOliveAgentRenameTags(fullResponse);
    const oliveagentDeletePaths = getOliveAgentDeleteTags(fullResponse);
    const oliveagentAddDependencyPackages = getOliveAgentAddDependencyTags(fullResponse);
    const oliveagentExecuteSqlQueries = chatWithApp.app.supabaseProjectId
      ? getOliveAgentExecuteSqlTags(fullResponse)
      : [];
    ...
    if (oliveagentExecuteSqlQueries.length > 0) {
      for (const query of oliveagentExecuteSqlQueries) {
        try {
      ...
      logger.log(`Executed ${oliveagentExecuteSqlQueries.length} SQL queries`);
    }

    // TODO: Handle add dependency tags
    if (oliveagentAddDependencyPackages.length > 0) {
      try {
        await executeAddDependency({
          packages: oliveagentAddDependencyPackages,
          message: message,
          appPath,
        });
      } catch (error) {
        errors.push({
          message: `Failed to add dependencies: ${oliveagentAddDependencyPackages.join(
            ", ",
          )}`,
          error: error,
        });
    ...
    for (const filePath of oliveagentDeletePaths) {
      const fullFilePath = safeJoin(appPath, filePath);
    ...
    for (const tag of oliveagentRenameTags) {
    ...
    for (const tag of oliveagentSearchReplaceTags) {
      ...
      try {
        if (!fs.existsSync(fullFilePath)) {
          // Do not show warning to user because we already attempt to do a <oliveagent-write> tag to fix it.
        ...
        const result = applySearchReplace(original, tag.content);
        if (!result.success || typeof result.content !== "string") {
          // Do not show warning to user because we already attempt to do a <oliveagent-write> and/or a subsequent <oliveagent-search-replace> tag to fix it.
          logger.warn(
    ...
    for (const tag of oliveagentWriteTags) {
      const filePath = tag.path;
    ...
    hasChanges =
      ...
      oliveagentAddDependencyPackages.length > 0;
    ...
    if (hasChanges) {
      ...
      if (oliveagentAddDependencyPackages.length > 0)
        changes.push(
          `added ${oliveagentAddDependencyPackages.join(", ")} package(s)`,
        );
      if (oliveagentExecuteSqlQueries.length > 0)
        changes.push(`executed ${oliveagentExecuteSqlQueries.length} SQL queries`);

      let message = chatSummary
        ? `[oliveagent] ${chatSummary} - ${changes.join(", ")}`
        : `[oliveagent] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of OliveAgent",
            amend: true,
          });
          logger.log(
            `Amend commit with changes outside of oliveagent: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of OliveAgent if needed.
          logger.error(
            `Failed to commit changes outside of oliveagent: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
    ...
    ${warnings
      .map(
        (warning) =>
          `<oliveagent-output type="warning" message="${warning.message}">${warning.error}</oliveagent-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<oliveagent-output type="error" message="${error.message}">${error.error}</oliveagent-output>`,
      )
      .join("\n")}
    `;
