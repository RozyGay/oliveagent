...
export function getOliveAgentAppPath(appPath: string): string {
  if (IS_TEST_BUILD) {
    const electron = getElectron();
    return path.join(electron!.app.getPath("userData"), "oliveagent-apps", appPath);
  }
  return path.join(os.homedir(), "oliveagent-apps", appPath);
}
