...
async function verifyReleaseAssets() {
  try {
    ...
    const owner = "oliveagent-sh";
    const repo = "oliveagent";
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "oliveagent-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `oliveagent-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `oliveagent-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `oliveagent-${version}.Setup.exe`,
      `oliveagent-darwin-arm64-${version}.zip`,
      `oliveagent-darwin-x64-${version}.zip`,
      `oliveagent_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      "RELEASES",
    ];
