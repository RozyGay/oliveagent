...
1. `oliveagent-{version}-1.x86_64.rpm` (Linux RPM)
2. `oliveagent-{version}-full.nupkg` (Windows NuGet package)
3. `oliveagent-{version}.Setup.exe` (Windows installer)
4. `oliveagent-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `oliveagent-darwin-x64-{version}.zip` (macOS Intel)
6. `oliveagent_{version}_amd64.deb` (Linux DEB)
