...
/**
 * A webpack loader that adds `data-oliveagent-*` attributes to JSX elements.
 */
export default function oliveagentTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
          ...
          try {
            ...
            const alreadyTagged = node.attributes?.some(
              (attr: any) =>
                ...
                attr.name?.name === "data-oliveagent-id",
            );
            if (alreadyTagged) return;

            // Build the oliveagent ID
            const loc = node.loc?.start;
            if (!loc) return;
            const oliveagentId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-oliveagent-id="${oliveagentId}" data-oliveagent-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[oliveagent-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[oliveagent-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[oliveagent-tagger] ERROR in ${this.resourcePath}:`, err);
