...
test("copy message content - basic functionality", async ({ po }) => {
  ...
  expect(clipboardContent).not.toContain("<oliveagent-");
});

test("copy message content - oliveagent-write conversion", async ({ po }) => {
  await po.setUp({ autoApprove: true });
  await po.importApp("minimal");
  ...
  // Should convert oliveagent-write to markdown format (flexible path matching)
  expect(clipboardContent).toContain("### File:");
  expect(clipboardContent).toContain("```");
  expect(clipboardContent).not.toContain("<oliveagent-write");
});
