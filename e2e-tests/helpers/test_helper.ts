...
export class PageObject {
  ...
  async setUpOliveAgentPro({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    await this.baseSetup();
    await this.goToSettingsTab();
    if (autoApprove) {
      await this.toggleAutoApprove();
    }
    await this.setUpOliveAgentProvider();
    await this.goToAppsTab();
  }
  ...
  async setUpOliveAgentProvider() {
    await this.page
      .locator("div")
      .filter({ hasText: /^OliveAgentNeeds Setup$/ })
      .nth(1)
      .click();
    await this.page.getByRole("textbox", { name: "Set OliveAgent API Key" }).click();
    await this.page
      .getByRole("textbox", { name: "Set OliveAgent API Key" })
      .fill("testoliveagentkey");
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[oliveagent-dump-path=*]]" is replaced with a placeholder path
      ...
      await this.page.evaluate(() => {
        ...
        messagesList.innerHTML = messagesList.innerHTML.replace(
          /\[\[oliveagent-dump-path=([^\]]+)\]\]/g,
          "[[oliveagent-dump-path=*]]",
        );
      });
    }
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[oliveagent-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[oliveagent-dump-path=([^\]]+)\]\]/g, "[[oliveagent-dump-path=*]]");
  ...
  getChatInput() {
    return this.page.locator(
      '[data-lexical-editor="true"][aria-placeholder="Ask OliveAgent to build..."]',
    );
  }
  ...
  async setUpAzure({ autoApprove = false }: { autoApprove?: boolean } = {}) {
    ...
    // so we don't need additional setup steps like setUpOliveAgentProvider
    await this.goToAppsTab();
  }
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "oliveagent-apps", appName);
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$oliveagentUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.OLIVEAGENT_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.OLIVEAGENT_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      ...
      const userDataDir = path.join(baseTmpDir, `oliveagent-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$oliveagentUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing oliveagent.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im oliveagent.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed oliveagent.exe");
        } catch (error) {
          console.warn(
            "Failed to kill oliveagent.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<oliveagent-file path="package\.json">[\s\S]*?<\/oliveagent-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
