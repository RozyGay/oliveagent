...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-oliveagent.tsx");
  ...
  await po.page.getByText("made-with-oliveagent.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithOliveAgent = ;");
  ...
  expect(editedFile).toContain("export const MadeWithOliveAgent = ;");
});
