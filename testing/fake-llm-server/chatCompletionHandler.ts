...
<oliveagent-write path="new/image/file.png" description="Uploaded image to codebase">
...
`;
    ...
    if (
      ...
      messageContent = `
<oliveagent-write path="src/bad-file.ts" description="Fix 2 errors and introduce a new error.">
...
x.nonExistentMethod2();
    ...
    if (
      ...
      // Fix errors in create-ts-errors.md and introduce a new error
      messageContent = `
<oliveagent-write path="src/bad-file.ts" description="Fix remaining error.">
// Import doesn't exist
// import NonExistentClass from 'non-existent-class';
...
x.toString(); // replaced with existing method
    ...
    if (
      ...
      messageContent = `
      Fixing the error...
      <oliveagent-write path="src/pages/Index.tsx">
      

import { MadeWithOliveAgent } from "@/components/made-with-oliveagent";
...
export default Index;
    ...
    if (
      ...
      typeof lastMessage.content === "string" &&
      lastMessage.content.startsWith(
        "There was an issue with the following `oliveagent-search-replace` tags.",
      )
    ) {
      if (lastMessage.content.includes("Make sure you use `oliveagent-read`")) {
        // Fix errors in create-ts-errors.md and introduce a new error
        messageContent =
          `
<oliveagent-read path="src/pages/Index.tsx"></oliveagent-read>

<oliveagent-search-replace path="src/pages/Index.tsx">
<<<<<<< SEARCH
...
` +
        ...
        messageContent =
          `
<oliveagent-write path="src/pages/Index.tsx" description="Rewrite file.">
// FILE IS REPLACED WITH FALLBACK WRITE.
    ...
    if (
      ...
      messageContent = `[[STRING_IS_FINISHED]]";</oliveagent-write>\nFinished writing file.`;
      messageContent += "\n\n" + generateDump(req);
    }
    const isToolCall = !!(
      lastMessage &&
...
function generateDump(req: Request) {
  ...
  try {
    ...
    return `[[oliveagent-dump-path=${dumpFilePath}]]`;
  } catch (error) {
